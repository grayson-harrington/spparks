...
void CreateSites::command(int narg, char **arg) {
    ...
    if (strcmp(arg[0], "box") == 0) {
        ...
        error->all(FLERR, "Illegal create_sites command");
    ...
    while (iarg < narg) {
        if (strcmp(arg[iarg], "value") == 0) {
            ...
            if (valueflag == DUMMY)
                error->all(FLERR,
                           "Must use value option before basis option "
            ...
            for (int i = ilo; i <= ihi; i++) {
                basisflag[i] = 1;
                if (valueflag == IARRAY)
                    basis_ivalue[i] = atoi(arg[iarg + 2]);
                else if (valueflag == DARRAY)
                    basis_dvalue[i] = atof(arg[iarg + 2]);
                count++;
            }
                ...
                for (m = 0; m < nbasis; m++) {
                    ...
                    if (valueflag == IARRAY) {
                        if (basisflag[m + 1])
                            ...
                            iarray[valueindex][nlocal - 1] = ivalue;
                    } else if (valueflag == DARRAY) {
                        if (basisflag[m + 1])
                            darray[valueindex][nlocal - 1] = basis_dvalue[m + 1];
                        else
                            darray[valueindex][nlocal - 1] = dvalue;
                    }
...
void CreateSites::random_sites() {
    ...
    for (tagint n = 1; n <= nrandom; n++) {
        ...
        if (latticeflag)
            applattice->add_site(n, x, y, z);
        else
            appoff->add_site(n, x, y, z);

        nlocal = app->nlocal;
        if (valueflag == IARRAY)
            iarray[valueindex][nlocal] = ivalue;
        else if (valueflag == DARRAY)
            darray[valueindex][nlocal] = dvalue;
    }
    ...
    for (i = 0; i < nlocal + nghost; i++) {
        ...
        while (j < numneigh[i]) {
            ...
            if (loc != hash.end()) {
                ...
                error->one(FLERR, "Ghost connection was not found");
        }
    }
